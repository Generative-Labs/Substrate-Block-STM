...
use crate::sync_wrapper::{ExplicitSyncWrapper, Mutex};
...
/// a dependency condition variable is set in ExecutionTaskType::Wakeup(DependencyCondvar): an
/// execution of a prior incarnation is waiting on it with a read dependency resolved (when
/// dependency was encountered, the status changed to Suspended, and suspended changed to Ready when
/// the dependency finished its execution). In this case the caller need not create a new execution
/// task, but just notify the suspended execution via the dependency condition variable.
///
...
/// this status during the transaction invariant checks, e.g., suspend(), resume(),
/// set_executed_status().
///
...
/// meanings, but in general the concept of 'wave' keeps track of the version number of the
/// validation.
///
...
///
/// 'maybe_max_validated_wave' records the maximum wave among successful validations of the
/// corresponding transaction, will be incremented upon successful validation (finish_validation).
/// Initialized as None.
///
...
///
/// Other than ValidationStatus, the 'wave' information is also recorded in 'validation_idx' and
/// 'commit_state'. Below is the description of the wave meanings and how they are updated. More
/// details can be found in the definition of 'validation_idx' and 'commit_state'.
///
...
/// required_wave <- validation_idx.wave; (otherwise, the last triggered wave is below and will
/// validate). (3) Upon validating a txn successfully, update txn's maybe_max_validated_wave <-
/// validation_idx.wave; (4) Upon trying to commit an executed txn, update commit_state.wave <-
/// txn's max_triggered_wave. (5) If txn's maybe_max_validated_wave >= max(commit_state.wave, txn's
/// required_wave), can commit the txn.
///
/// Remark: commit_state.wave is updated only with max_triggered_wave but not required_wave. This is
/// because max_triggered_wave implies that this wave of validations was required for all higher
/// transactions (and is set as a part of decrease_validation_idx), while required_wave is set for
/// the transaction only (when a validation task is returned to the caller). Moreover, the code is
/// structured in a way that decrease_validation_idx is always called for txn_idx + 1 (e.g. when
/// aborting, there is no need to validate the transaction before re-execution, and in
/// finish_execution, even if there is a need to validate txn_idx, it is returned to the caller
/// directly, which is done so as an optimization and also for uniformity).
#[derive(Debug)]
...
impl ValidationStatus {
    pub fn new() -> Self {
        ValidationStatus { max_triggered_wave: 0, required_wave: 0, maybe_max_validated_wave: None }
    }
}
...
pub struct Scheduler {
    ...
    /// The threads increment the index and attempt to create an execution task for the
    /// corresponding transaction, if the status of the txn is 'Ready'. This implements a
    /// counting-based concurrent ordered set. It is reduced as necessary when transactions
    /// become ready to be executed, in particular, when execution finishes and dependencies are
    /// resolved.
...
impl Scheduler {
    pub fn new(num_txns: TxnIndex) -> Self {
        ...
        Self {
            num_txns,
            txn_dependency: (0..num_txns).map(|_| CachePadded::new(Mutex::new(Vec::new()))).collect(),
    ...
    pub fn add_to_commit_queue(&self, txn_idx: u32) {
        self.commit_queue.push(txn_idx).expect("Pushing to the commit_queue should never fail");
    }
    ...
    pub fn try_commit(&self) -> Option<TxnIndex> {
        ...
        if let Some(status) = self.txn_status[*commit_idx as usize].0.try_upgradable_read() {
    ...
    pub fn next_task(&self) -> SchedulerTask {
        loop {
            ...
            let (idx_to_validate, wave) = Self::unpack_validation_idx(self.validation_idx.load(Ordering::Acquire));

            let idx_to_execute = self.execution_idx.load(Ordering::Acquire);

            let prefer_validate =
                idx_to_validate < min(idx_to_execute, self.num_txns) && !self.never_executed(idx_to_validate);
            ...
            if prefer_validate {
                if let Some((txn_idx, incarnation, wave)) = self.try_validate_next_version(idx_to_validate, wave) {
                    return SchedulerTask::ValidationTask(txn_idx, incarnation, wave);
                }
            }

            if idx_to_execute < self.num_txns {
                if let Some((txn_idx, incarnation, execution_task_type)) = self.try_execute_next_version() {
    ...
    pub fn wait_for_dependency(&self, txn_idx: TxnIndex, dep_txn_idx: TxnIndex) -> DependencyResult {
    ...
    pub fn finish_validation(&self, txn_idx: TxnIndex, wave: Wave) {
        ...
        validation_status.maybe_max_validated_wave =
            Some(validation_status.maybe_max_validated_wave.map_or(wave, |prev_wave| max(prev_wave, wave)));
    }
    ...
    pub fn finish_execution(
        ...
        let (cur_val_idx, mut cur_wave) = Self::unpack_validation_idx(self.validation_idx.load(Ordering::Acquire));
    ...
    /// 1. There is a module publishing txn that has read/write intersection with any txns even
...
impl Scheduler {
    ...
    fn halt_transaction_execution(&self, txn_idx: TxnIndex) {
        let mut status = self.txn_status[txn_idx as usize].0.write();
        {
            // Only transactions with status Suspended or Ready may have the condition variable of pending
            // threads.
            match &*status {
    ...
    fn unpack_validation_idx(validation_idx: u64) -> (TxnIndex, Wave) {
        ((validation_idx & TXN_IDX_MASK) as TxnIndex, (validation_idx >> 32) as Wave)
    }
    ...
    fn decrease_validation_idx(&self, target_idx: TxnIndex) -> Option<Wave> {
        ...
        if let Ok(prev_val_idx) = self.validation_idx.fetch_update(Ordering::SeqCst, Ordering::Acquire, |val_idx| {
            ...
            if txn_idx > target_idx {
                ...
                validation_status.max_triggered_wave = max(validation_status.max_triggered_wave, wave + 1);
    ...
    fn never_executed(&self, txn_idx: TxnIndex) -> bool {
        ...
        matches!(
            *status,
            ExecutionStatus::Ready(0, _) | ExecutionStatus::Executing(0) | ExecutionStatus::Suspended(0, _)
        )
    }
    ...
    fn try_validate_next_version(
        ...
        if self
            .validation_idx
            .compare_exchange(curr_validation_idx, next_validation_idx, Ordering::SeqCst, Ordering::Acquire)
            .is_ok()
        {
            ...
            return self.is_executed(idx_to_validate, false).map(|incarnation| (idx_to_validate, incarnation, wave));
        }
    ...
    fn try_execute_next_version(&self) -> Option<(TxnIndex, Incarnation, ExecutionTaskType)> {
        ...
        self.try_incarnate(idx_to_execute)
            .map(|(incarnation, execution_task_type)| (idx_to_execute, incarnation, execution_task_type))
    }
    ...
    fn resume(&self, txn_idx: TxnIndex) {
        ...
        if let ExecutionStatus::Suspended(incarnation, dep_condvar) = &*status {
            *status = ExecutionStatus::Ready(*incarnation, ExecutionTaskType::Wakeup(dep_condvar.clone()));
        } else {
            unreachable!();
        }
    }
