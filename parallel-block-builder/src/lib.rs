//! `parallel-block-builder` crate
//!
//! This crate provides the implementation of the parallel block builder.
...
pub use wp_block_builder::ParallelBlockBuilder as BlockBuilderApi;
	...
	C::Api: BlockBuilderApi<Block>,
{
	...
	pub fn batch_push(&mut self, xts: sp_std::vec::Vec<<Block as BlockT>::Extrinsic>) -> Result<(), Error> {
		...
		self.api.execute_in_transaction(|api| {
			...
			match res {
				Ok(Ok(_)) => {
                    extrinsics.extend(xts[1..].iter().cloned());
